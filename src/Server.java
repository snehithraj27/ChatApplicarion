...
public class Server {
	...
	public static void writeFile(String msg) {
		try {
			File f1 = new File("/Users/snehithraj/eclipse-workspace/ChatRoomSystem/src/ServerDB.txt");
			if (!f1.exists()) {
				f1.createNewFile();
			}
		...
		public void run() {
			try {
				...
				while (true) {
					...
					writeFile("From client" + name + ":" + input + "\n");
					serverMessageArea.append("From client" + name + ":" + input + "\n");
				}
			} catch (IOException e) {
				System.out.println(e);
			} finally {
				// This client disconnected. Remove its name and its print
				...
				try {
					socket.close();
					writeFile("Chat Server is disconnected\n");
				} catch (IOException e) {
					System.out.println(e);
				}
			}
